<?php
class Category2Dao extends DaoBase {
...
//        $this->setFetchClass("RentalCategory");
	...
	public function getRentalCategorySort() {
		...
		$sql="select c.name,c.id,c.parentid,c.v_order,c.cflag
				FROM mst_category2 AS c
				where c.parentid is null order by c.v_order ";
		...
		foreach($rs as $res){
        	...
        	//該当の親に紐づいている中カテゴリ
	       	$sql="SELECT c.name, c.id,c.parentid,c.v_order,c.cflag
					FROM mst_category2 AS c
					where c.parentid=".$res->getId()." ORDER BY c.parentid,c.v_order";
	        		...
	        		foreach($nrs as $nres){
			        	...
			        	$sql="SELECT c.name, c.id,c.parentid,c.v_order,c.cflag
							FROM mst_category2 AS c
							where c.parentid=".$nres->getId()." ORDER BY c.parentid,c.v_order";
...
//		if($parent_id=="") $parent_id=NULL;
		...
		$name = $this->db->quote(htmlspecialchars(addslashes($category->getName())));
		$order = $this->db->quote(htmlspecialchars(addslashes($category->getOrder())));
		$regdate = $this->db->quote(htmlspecialchars(addslashes($category->getRegdate())));
		$cflag = $this->db->quote(htmlspecialchars(addslashes($category->getCflag())));
...
*/
		...
		if($category->getOrder()){
			$setTmp[]=" `v_order` =".addslashes($category->getOrder());
		}
		if($category->getName()){
			$setTmp[]=" name ='".addslashes($category->getName())."'";
		}
...
//echo $sql;
	 ...
	 *
	 */
	public function getCatgoryControll($id,$catgory_id) {
		$id = $this->db->quote($id);
		$catgory_id = $this->db->quote($catgory_id);

       $sql = "SELECT
                    a.*
                FROM
                    tbl_item_categoryControll  a
                WHERE
                    item_id ='".addslashes($id)."' and category_id = '".addslashes($catgory_id)."' and item_flag=2
                ";
//        $this->setFetchClass("RentalCategory");
        $rs = $this->executeQuery($sql);
