...
$ss=date("s");
	...
	for($i=0;$i<count($arr);$i++){
		...
		//時間等
		if($arr[$i]['step_time']==1){//日
			if($sw==1){
				$chkdttime=date("Y-m-d H:i:s", mktime($hh,$ii,$ss,$mm,$dd+$step_long,$yy));
				$chkdt=date("Y-m-d", mktime($hh,$ii,$ss,$mm,$dd+$step_long,$yy));
				$chktime=date("H:i:s", mktime($hh,$ii,$ss,$mm,$dd+$step_long,$yy));
...
//			send_mail("ksuzuki@apice-tec.co.jp", MAIL_FROM, MAIL_FROM_NAME, $subject, $mailBody,"",RETURN_EMAIL);
			...
			// send_mail("alam@mailinator.com", MAIL_FROM, MAIL_FROM_NAME, $memArr[0]['email'], $mailBody);
			 exit();
