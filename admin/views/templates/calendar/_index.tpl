							...
							<table class="time-table-app">
								...
								{foreach from=$arrRsv item=item}
									...
									{section name=foo start=10 loop=21}
										...
										<td><div class="quad" onClick="dispReserve('{$item.shop_no}','{$ji}','00','{$reserve_date}','{$item.dt}');"></div><div class="quad" onClick="dispReserve('{$item.shop_no}','{$ji}','15','{$reserve_date}');"> </div><div class="quad" onClick="dispReserve('{$item.shop_no}','{$ji}','30','{$reserve_date}');"> </div><div class="quad" onClick="dispReserve('{$item.shop_no}','{$ji}','45','{$reserve_date}');"> </div>
											{if $rsv.$ji}
												<div onclick="nextDetail({$rsv.$ji.no},'{$item.dt}');" onmouseover="dispDetail({$key},{$ji});" class="app-preview type{$rsv.$ji.rsv_type}" style="left:{$rsv.$ji.px}%;background-color : {$rsv.$ji.bg}" >
													<p>{$rsv.$ji.start_time|date_format:"%H:%M"}<br><span class="bold">{if $rsv.$ji.name}{$rsv.$ji.name}様{/if}</span><br>{$rsv.$ji.menu_name}</p>
												</div>
												<div class="tooltip clearfix"  id="re{$key}{$ji}">
												<dl><!-- マウスオーバー字に吹き出しとして表示させる情報 -->
													<dt>予約時間</dt>
													<dd>{$rsv.$ji.start_time|date_format:"%H:%M"}</dd>
													<dt>お名前</dt>
													<dd>{if $rsv.$ji.name}{$rsv.$ji.name}様{/if}</dd>
													<dt>お電話番号</dt>
													<dd>{if $rsv.$ji.name}{$rsv.$ji.tel}{/if}</dd>
													<dt>メニュー</dt>
													<dd>{if $rsv.$ji.name}{$rsv.$ji.menu_name}{/if}</dd>
													<dt>予約番号</dt>
													<dd>{$rsv.$ji.reserve_no}</dd>
												</dl>
												</div>
											{else}
