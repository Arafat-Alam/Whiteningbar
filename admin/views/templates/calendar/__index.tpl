...
{/literal}
{foreach from=$arrRsv item=item}{foreach from=$item.rsv key=key item=rsv}{section name=foo start=10 loop=21}{assign var="ji" value=$smarty.section.foo.index}
...
{literal}
	...
	grid_x = $("#tablecontainer").width() / 44;
	grid_y = 51;
		...
		<div class="content">
			...
			<!-- <button onClick="window.close();">閉じる</button>-->
				<div class="time-table-box clear">
					...
					<div class="app-daily-total"><input type="radio" name="mmode" id="mmoderadio1" value="1" checked="checked" onclick="toggledrag(1);" /> <label for="mmoderadio1" style="display:inline;">詳細表示</label>　<input type="radio" name="mmode" id="mmoderadio2" value="2" onclick="toggledrag(2);" /> <label for="mmoderadio2" style="display:inline;">移動モード</label></div>
					<div class="clear">
							...
							<table class="time-table-app">
								...
								{foreach from=$arrRsv name=aaa item=item}
									...
									{section name=foo start=10 loop=21}
										...
										<td class="drop">
										{/if}
										<div class="quad" onClick="dispReserve('{$item.shop_no}','{$ji}','00','{$reserve_date}','{$item.dt}');"></div><div class="quad" onClick="dispReserve('{$item.shop_no}','{$ji}','15','{$reserve_date}','{$item.dt}');"> </div><div class="quad" onClick="dispReserve('{$item.shop_no}','{$ji}','30','{$reserve_date}','{$item.dt}');"> </div><div class="quad" onClick="dispReserve('{$item.shop_no}','{$ji}','45','{$reserve_date}','{$item.dt}');"> </div>

									{foreach from=$rsv.$ji key=bango item=jjj}
									{if isset($jjj.no)}
												<div onclick="if(dragmode == 1)nextDetail({$jjj.no},'{$item.dt}');" onmouseover="dispDetail({$key},{$ji});" class="app-preview type{$jjj.rsv_type}" style="left:{$jjj.px}%;background-color : {$jjj.bg};" id="red{$jjj.no}_{$ji}_{$item.shop_no}" >
													<p>{$jjj.start_time|date_format:"%H:%M"}<br><span class="bold">{if $jjj.name}{$jjj.name}{elseif $jjj.name_kana}{$jjj.name_kana}様{/if}{if $jjj.numb>1}({$jjj.numb}){/if}</span><br>{$jjj.menu_name}</p>
												</div>
												<div class="tooltip_container"  id="re{$jjj.no}_{$ji}_{$item.shop_no}" style="display: none;">
